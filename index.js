...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `𝐒 𝐀 𝐈 𝐔  + 1 𝐂 𝐎 𝐑 𝐍 𝐎 😻😀 @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '𝘾𝙖𝙡𝙢𝙖 𝙚𝙨𝙩𝙤𝙪 𝙛𝙖𝙯𝙚𝙣𝙙𝙤, 𝙚𝙣𝙦𝙪𝙖𝙣𝙩𝙤 𝙞𝙨𝙨𝙤 𝙢𝙖𝙢𝙖 𝙤 𝙖𝙙𝙢😘😈,
				success: 'SATISFEITO?¿ ENTÃO ME MAMA',
				error: {
				...
				only: {
					...
					ownerB: '❌ M𝐭𝐞𝐦 𝐞𝐬𝐬𝐞 𝐜𝐨𝐦𝐚𝐧𝐝𝐨 𝐚𝐞? 𝐋ê 𝐝𝐢𝐫𝐞𝐭𝐨 𝐟𝐢😘😈
					admin: 'MENSAGEM QUE APARECE QUANDO ALGUEM TENTA USAR UM COMANDO QUE SÓ PODE SER USADO PELO DONO DO BOT❌',
					Badmin: 'VOCE NÃO É ADM, SE FUDEU'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5527999490287@s.whatsapp.net"] // COLOQUE SEU NÚMERO AQUI SEM ESPAÇOS
	...
	//AQUI NAO PRECISA MUDAR
			...
			case 'tagall':
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('QUEM É O CORNO QUE ADD AQUI?😘😈'') 
					if (args[0].startsWith('08')) return reply('ESCREVE DIRETO ANIMAL😡')
					try {
...
//NAO FUNCIONA MAIS	
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('''•¸.⸵░⃟🌵𝐵𝛯𝑀 ꪜ 𝐼𝑁𝐷𝑂 ₐᵒ 𝐷𝜟𝑳𝑳𝜟𝑺•🌵 ⃟ ░
`?𝐄 𝐀𝐏𝐑𝐄𝐒𝐄𝐍𝐓𝐀 𝐄𝐌 𝟏 𝐌𝐈𝐍 ou【 𝑩𝑨𝙉】🍨🌵🍁🍨🌵🍁🍨🍁🌵🍨🌵🍁
😏ᶜⁱᵈᵃᵈᵉ..
🙈ᴺᵒᵐᵉ..
🚨ᴵᵈᵃᵈᵉ..
☯️ˢⁱᵍⁿᵒ..
